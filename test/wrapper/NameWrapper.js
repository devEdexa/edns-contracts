...
describe('Name Wrapper', () => {
  ...
  before(async () => {
    ...
    expect(await EnsRegistry.owner(namehash('tedx'))).to.equal(
      BaseRegistrar.address,
    )
  })
  ...
  describe('wrap()', () => {
    ...
    it('Does not allow wrapping .tedx 2LDs.', async () => {
      const label = 'wrapped'
      const labelHash = labelhash(label)
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await EnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrap(encodeName('wrapped.tedx'), account2, EMPTY_ADDRESS),
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC', async () => {
      const label = 'test'
      const labelHash = labelhash(label)
      const wrappedTokenId = namehash(label + '.tedx')
      const subLabel = 'sub'
      const subLabelHash = labelhash(subLabel)
      const subWrappedTokenId = namehash(`${subLabel}.${label}.tedx`)
      ...
      await NameWrapper2.wrap(
        encodeName(`${subLabel}.${label}.tedx`),
        account2,
        EMPTY_ADDRESS,
      )
  ...
  describe('unwrap()', () => {
    ...
    it('Will not allow previous owner to unwrap name when name expires', async () => {
      ...
      await expect(
        ...
        `Unauthorised("${namehash('sub.unwrapped.tedx')}", "${account}")`,
      )
    })
    ...
    it('Will not unwrap .tedx 2LDs.', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('tedx'), labelhash('unwrapped'), account),
      ).to.be.revertedWith('IncompatibleParent()')
    })
    ...
    it('Will not allow to unwrap with PCC/CU burned if expired', async () => {
      ...
      expect(await EnsRegistry.owner(namehash('sub.awesome.tedx'))).to.equal(
        NameWrapper.address,
      )

      await expect(
        NameWrapper.unwrap(namehash('awesome.tedx'), labelhash('sub'), account),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.awesome.tedx')}", "${account}")`,
      )
    })
    ...
    it('Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('abc.tedx'), labelhash('sub'), account),
      ).to.be.revertedWith(`OperationProhibited("${namehash('sub.abc.tedx')}")`)
    })
  ...
  describe('wrapETH2LD()', () => {
    ...
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.tedx')
    ...
    it('Cannot wrap a name if the owner has not authorised the wrapper with the .tedx registrar.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .tedx registrar', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD(
        label,
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.tedx'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.tedx'),
          encodeName('wrapped2.tedx'),
          account2,
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      ;[, fuses] = await NameWrapper.getData(namehash('sub.wrapped2.tedx'))
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('wrapped2.tedx'))
    ...
    it('Allows an account approved by the owner on the .tedx registrar to wrap a name.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
  ...
  describe('unwrapETH2LD()', () => {
    ...
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.tedx'))).to.equal(
        account,
      )
      await NameWrapper.unwrapETH2LD(labelHash, account, account)
      // transfers the controller on the .tedx registrar to the target address.
      expect(await EnsRegistry.owner(namehash('unwrapped.tedx'))).to.equal(
        account,
      )
      //Transfers the registrant on the .tedx registrar to the target address
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.tedx'), account)
    })
  ...
  describe('approve()', () => {
    ...
    const subWrappedTokenId = namehash('sub.' + label + '.tedx')
    let parentExpiry
    let result
    before(async () => {
      result = await ethers.provider.send('evm_snapshot')
    ...
    it('Does not allow approved accounts to extend expiry when expired', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.subdomain.tedx')}")`,
      )
    })
    ...
    it('Approved address cannot unwrap .tedx', async () => {
      await NameWrapper.approve(account2, wrappedTokenId)
      await expect(
    ...
    it('Approved address cannot unwrap non .tedx', async () => {
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
        'sub',
        account,
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.tedx'))
      await expect(
        NameWrapper2.unwrap(wrappedTokenId, labelhash('sub'), account2),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.subdomain.tedx')}", "${account2}")`,
      )
    })
    ...
    it('Approval is cleared on unwrap()', async () => {
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.tedx'))
      expect(
        await NameWrapper.getApproved(namehash('sub.subdomain.tedx')),
      ).to.equal(account2)
  ...
  describe('getApproved()', () => {
    ...
    it('Returns returns zero address when ownerOf() is zero', async () => {
      expect(await NameWrapper.ownerOf(namehash('unminted.tedx'))).to.equal(
        EMPTY_ADDRESS,
      )
      expect(await NameWrapper.getApproved(namehash('unminted.tedx'))).to.equal(
        EMPTY_ADDRESS,
      )
    })
  ...
  describe('upgrade()', () => {
    describe('.tedx', () => {
      const encodedName = encodeName('wrapped2.tedx')
      const label = 'wrapped2'
      const labelHash = labelhash(label)
      const nameHash = namehash(label + '.tedx')

      it('Upgrades a .tedx name if sender is owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * DAY)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      ...
      it('Upgrades a .tedx name if sender is authorised by the owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * DAY)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
        await NameWrapper.setApprovalForAll(account2, true)
    ...
    describe('other', () => {
      ...
      const name = label + '.' + parentLabel + '.tedx'
      const parentLabelHash = labelhash(parentLabel)
      const parentHash = namehash(parentLabel + '.tedx')
  ...
  describe('setFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.tedx')
    ...
    it('cannot burn fuses as the previous owner of a .tedx when the name has expired', async () => {
      await BaseRegistrar.register(labelhash('abc'), account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD('abc', account, CANNOT_UNWRAP, EMPTY_ADDRESS)
      ...
      await expect(
        NameWrapper.setFuses(namehash('abc.tedx'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('abc.tedx')}", "${account}")`,
      )
    })

    it('cannot burn fuses as a previous owner of a non .tedx when the name has expired', async () => {
      await BaseRegistrar.register(labelhash('abc'), account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD('abc', account, CANNOT_UNWRAP, EMPTY_ADDRESS)
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.tedx'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.abc.tedx')}", "${account}")`,
      )
    })
    ...
    it('Will not allow burning fuses of subdomains if CANNOT_UNWRAP has not been burned', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.tedx'), CANNOT_TRANSFER),
      ).to.be.revertedWith(
        `OperationProhibited("0x5f1471f6276eafe687a7aceabaea0bce02fafaf1dfbeb787b3725234022ee294")`,
      )
    })

    it('Will not allow burning fuses of .tedx names unless CANNOT_UNWRAP is also burned.', async () => {
      await BaseRegistrar.register(tokenId, account, 1 * DAY)
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.tedx')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await EnsRegistry.owner(namehash('creatable.fuses2.tedx')),
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.tedx')),
      ).to.equal(account)
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('uncreatable.fuses2.tedx')}")`,
      )
    })
  })
  ...
  describe('extendExpiry()', () => {
    ...
    it('Allows parent owner to set expiry without CAN_EXTEND_EXPIRY burned', async () => {
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('sub.fuses.tedx'))
    ...
    it('Does not allow owner of .tedx 2LD to set expiry', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)

      let [, fuses, expiry] = await NameWrapper.getData(namehash('fuses.tedx'))

      expect(fuses).to.equal(IS_DOT_ETH | PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)

      await expect(
        NameWrapper.extendExpiry(namehash('tedx'), tokenId, expiry),
      ).to.be.revertedWith(`OperationProhibited("${wrappedTokenId}")`)
    })
    ...
    it('Does not allow .tedx 2LD owner to set expiry on child if the .tedx 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Allows child owner to set expiry if parent .tedx 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Does not allow extendExpiry() to be called on unwrapped names', async () => {
      ...
      let registryOwner = await EnsRegistry.owner(namehash('sub.fuses.tedx'))
    ...
    it('Emits Expiry Extended event', async () => {
      ...
      await expect(tx)
        ...
        .withArgs(namehash(`sub.${label}.tedx`), parentExpiry.add(GRACE_PERIOD))
    })
  })
  ...
  describe('setChildFuses()', () => {
    ...
    it('Does not allow parent owners to burn parent controlled fuses after burning PCC', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.fuses.tedx')}")`,
      )
    })
    ...
    it('Does not allow non-parent owners to set child fuses', async () => {
      const subWrappedTokenId = namehash('sub.fuses.tedx')
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
    ...
    it('should not allow .tedx to call setChildFuses()', async () => {
      await registerSetupAndWrapName('fuses', account, 0)
      ...
      ).to.be.revertedWith(`Unauthorised("${namehash('tedx')}", "${account}")`)
    })
  ...
  describe('setSubnodeOwner()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the ENS registry.', async () => {
      ...
      expect(await EnsRegistry.owner(namehash(`sub.${label}.tedx`))).to.equal(
        NameWrapper.address,
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.tedx`))).to.equal(
        account,
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
    ...
    it('Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`${label2}.${label}.tedx`)}")`,
      )
    })
    ...
    it('Does not allow IS_DOT_ETH to be burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`sub.${label}.tedx`)}")`,
      )
    })
    ...
    it('Unwrapping within an external contract does not create any state inconsistencies', async () => {
      ...
      // set self as sub.test.tedx owner
      await NameWrapper.setSubnodeOwner(
        namehash('test.tedx'),
        'sub',
        account,
    ...
    it('Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent', async () => {
      ...
      const subWrappedTokenId = namehash(`${subLabel}.${label}.tedx`)
      const subWrappedTokenId2 = namehash(`${subLabel2}.${label}.tedx`)
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(labelHash)
      // Confirm that the name is wrapped
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
  ...
  describe('setSubnodeRecord()', async () => {
    ...
    it('Fuses will remain 0 if expired', async () => {
      ...
      const [, fuses] = await NameWrapper.getData(namehash(`sub.${label}.tedx`))
      expect(fuses).to.equal(0)
    })
    ...
    it('Sets the appropriate values on the ENS registry', async () => {
      ...
      const node = namehash(`sub.${label}.tedx`)
  ...
  describe('setRecord()', () => {
    ...
    it('Setting the owner to 0 reverts if CANNOT_UNWRAP is burned', async () => {
      ...
      const wrappedTokenId2 = namehash('setrecord2.tedx')
      const subWrappedTokenId = namehash('sub.setrecord2.tedx')
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId2,
        'sub',
        account,
    ...
    it('Setting the owner to 0 on a .tedx reverts', async () => {
      await registerSetupAndWrapName('setrecord2', account, 0)
      const wrappedTokenId2 = namehash('setrecord2.tedx')
  ...
  describe('getData', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.tedx`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.tedx`)
  ...
  describe('registerAndWrapETH2LD()', () => {
    ...
    it('Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.', async () => {
      ...
      ).to.be.revertedWith(`OperationProhibited("${namehash(label + '.tedx')}")`)
    })
  ...
  describe('isWrapped(bytes32 node)', () => {
    ...
    it('identifies a wrapped .tedx name', async () => {
      expect(await NameWrapper['isWrapped(bytes32)'](wrappedTokenId)).to.equal(
        true,
      )
    })

    it('identifies an expired .tedx name as unwrapped', async () => {
      const label = 'expired'
      const wrappedTokenId = namehash(label + '.tedx')
    ...
    it('identifies an eth name registered on old controller as unwrapped', async () => {
      ...
      const labelHash = labelhash(label)
      const tokenId = namehash('oldcontroller.tedx')
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
    ...
    it('identifies an unregistered .tedx name as unwrapped', async () => {
      const tokenId = namehash('abcdefghijklmnop.tedx')
      expect(await NameWrapper['isWrapped(bytes32)'](tokenId)).to.equal(false)
    })
    ...
    it('identifies a wrapped subname', async () => {
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
      const subTokenId = namehash('sub.something.tedx')
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const sub1Domain = sub1Label + '.' + label + '.tedx' // sub1.base.tedx
      ...
      const sub2Domain = sub2Label + '.' + sub1Domain // sub2.sub1.base.tedx
      ...
      const sub3Label = 'tedx'
      const sub3LabelHash = labelhash(sub3Label)
      const sub3Domain = sub3Label + '.' + sub2Domain // eth.sub2.sub1.base.tedx
  ...
  describe('Grace period tests', () => {
    ...
    const wrappedTokenId = namehash(label + '.tedx')
    const subLabel = 'sub'
    const subTokenId = namehash(subLabel + '.' + label + '.tedx')
    let parentExpiry
    before(async () => {
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      ;[, , parentExpiry] = await NameWrapper.getData(wrappedTokenId)
      ...
      // move .tedx name to expired and be within grace period
      await evm.advanceTime(2 * DAY)
      await evm.mine()
    ...
    it('When a .tedx name is in grace period it cannot call setSubnodeOwner', async () => {
      await expect(
        NameWrapper.setSubnodeOwner(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .tedx name is in grace period it cannot call setSubnodeRecord', async () => {
      await expect(
        NameWrapper.setSubnodeRecord(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .tedx name is in grace period it cannot call setRecord', async () => {
      await expect(
        NameWrapper.setRecord(wrappedTokenId, account2, EMPTY_ADDRESS, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .tedx name is in grace period it cannot call safeTransferFrom', async () => {
      await expect(
        NameWrapper.safeTransferFrom(
          account,
          account2,
    ...
    it('When a .tedx name is in grace period it cannot call batchSafeTransferFrom', async () => {
      await expect(
        NameWrapper.safeBatchTransferFrom(
          account,
          account2,
    ...
    it('When a .tedx name is in grace period it cannot call setResolver', async () => {
      await expect(
        NameWrapper.setResolver(wrappedTokenId, EMPTY_ADDRESS),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .tedx name is in grace period it cannot call setTTL', async () => {
      await expect(NameWrapper.setTTL(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .tedx name is in grace period it cannot call setFuses', async () => {
      await expect(NameWrapper.setFuses(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .tedx name is in grace period it cannot call setChildFuses', async () => {
      await expect(
        NameWrapper.setChildFuses(wrappedTokenId, labelhash('sub'), 0, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .tedx name is in grace period, unexpired subdomains can call setFuses', async () => {
      await NameWrapper2.setFuses(subTokenId, CANNOT_UNWRAP)
      const [, fuses] = await NameWrapper.getData(subTokenId)
      expect(fuses).to.equal(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)
    })

    it('When a .tedx name is in grace period, unexpired subdomains can transfer', async () => {
    ...
    it('When a .tedx name is in grace period, unexpired subdomains can set resolver', async () => {
      await NameWrapper2.setResolver(subTokenId, account)
      expect(await EnsRegistry.resolver(subTokenId)).to.equal(account)
    })

    it('When a .tedx name is in grace period, unexpired subdomains can set ttl', async () => {
      await NameWrapper2.setTTL(subTokenId, 100)
      expect(await EnsRegistry.ttl(subTokenId)).to.equal(100)
    })

    it('When a .tedx name is in grace period, unexpired subdomains can call setRecord', async () => {
    ...
    it('When a .tedx name is in grace period, unexpired subdomains can call setSubnodeOwner', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      expect(await NameWrapper.ownerOf(namehash('sub2.sub.test.tedx'))).to.equal(
        account2,
      )
    })

    it('When a .tedx name is in grace period, unexpired subdomains can call setSubnodeRecord', async () => {
      await NameWrapper2.setSubnodeRecord(
        subTokenId,
        'sub2',
        account2,
    ...
    it('When a .tedx name is in grace period, unexpired subdomains can call setChildFuses if the subdomain exists', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      await NameWrapper2.setChildFuses(subTokenId, labelhash('sub2'), 0, 100)
      const [owner, fuses, expiry] = await NameWrapper.getData(
        namehash('sub2.sub.test.tedx'),
      )
      expect(owner).to.equal(account2)
      expect(expiry).to.equal(100)
      expect(fuses).to.equal(0)
  ...
  describe('Registrar tests', () => {
    ...
    const wrappedTokenId1 = namehash('sub1.tedx')
    ...
    it('Reverts when attempting to call token owner protected function on an unwrapped name', async () => {
      ...
      // XXX: note that at this step, the hackler should use the current .tedx
      // registrar to directly register `sub1.tedx` to himself, without wrapping
      // the name.
      ...
      //
      // XXX: corrently, `sub1.tedx` becomes a normal node
      await EnsRegistryH.setOwner(wrappedTokenId1, NameWrapper.address)

      // create `sub2.sub1.tedx` to the victim user with `PARENT_CANNOT_CONTROL`
      // burnt.
      await expect(
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .tedx name', async () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap a non .tedx name', async () => {
    ...
    it('When .tedx name expires, it is untransferrable', async () => {
      await BaseRegistrar.register(labelhash(label), account, 1 * DAY)
      await NameWrapper.wrapETH2LD(label, account, 0, EMPTY_ADDRESS)
  ...
  describe('Implicit Unwrap tests', () => {
    ...
    const wrappedTokenId2 = namehash('sub2.sub1.tedx')
    ...
    it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
      ...
      // create `sub2.sub1.tedx` w/o fuses burnt
      await NameWrapperH.setSubnodeOwner(
        wrappedTokenId1,
        label2,
        hacker,
      ...
      // XXX: PREPARE HACK!
      // set `EnsRegistry.owner` of `sub1.tedx` as the hacker himself.
      ...
      // set `sub2.sub1.tedx` to the victim user w fuses burnt
    ...
    it('Renewing a wrapped, but expired name .tedx in the wrapper, but unexpired on the registrar resyncs expiry', async () => {
      await NameWrapper.registerAndWrapETH2LD(
        label1,
        account,
        1 * DAY,
